...
const RULES: { pattern: RegExp; category: RecommendedCategory; label: string; modelId: string }[] = [
  { pattern: /画|绘|图片|插画|海报|封面|logo|头像|壁纸|draw|paint|image|illustration|poster|avatar|wallpaper|icon|thumbnail|banner|infographic|sketch|portrait/i, category: "image", label: "DALL-E 3", modelId: "dall-e-3" },
  { pattern: /视频|短片|动画|影片|运镜|分镜|video|clip|animation|film|motion|cinematic|storyboard|footage|trailer/i, category: "video", label: "Sora", modelId: "sora" },
  ...
  { pattern: /代码|编程|函数|调试|重构|bug|code|program|function|debug|refactor|implement|algorithm|api|sdk|compile|deploy|script/i, category: "code", label: "DeepSeek V3", modelId: "deepseek-chat" },
